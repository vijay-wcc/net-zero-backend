...
def get_organizations(db: Session, skip: int = 0, limit: int = 100) -> list[Type[Organization]]:
...
def update_organization(db: Session, org_id: str, organization_in: OrganizationUpdate) -> Optional[Type[Organization]]:
...
def delete_organization(db: Session, org_id: str) -> Optional[Type[Organization]]:
    db_organization = db.query(Organization).filter(Organization.org_id == org_id).first()
    if db_organization is None:
        return None
